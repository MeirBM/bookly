...
public class BusinessController {
    ...
    @Operation(summary = "Set or clear the business logo; a blank URL clears it")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "The business, with its logo as it now stands"),
            @ApiResponse(responseCode = "400", description = "Not an absolute http(s) URL"),
            @ApiResponse(responseCode = "403",
                    description = "Not a member — returned identically when the business does not exist")})
    public BusinessResponse setLogo(@PathVariable UUID businessId,
                                    @Valid @RequestBody UpdateBusinessLogoRequest request) {
        return businessService.setLogo(businessId, request);
    }
}
